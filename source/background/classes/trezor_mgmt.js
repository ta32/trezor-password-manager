...
class TrezorMgmt {
  constructor(bgStore, tc) {
    ...
    this.trezorConnect.init({
      ...
      connectSrc: "moz-extension://0131fc0f-55c6-43d2-85e2-4a40ec02fb7c/",
      iframeSrc: "moz-extension://0131fc0f-55c6-43d2-85e2-4a40ec02fb7c/iframe.html",
      debug: true,
      webusb: false,
      popup: false,
  ...
  _handleTrezorError(error, operation, fallback) {
    ...
    if (error instanceof SyntaxError && operation === 'decEntry') {
      this._cryptoData.keyPhrase = this._displayOldKey(
        this._cryptoData.title,
        this._cryptoData.username
      );
      this._retryWrongPin.op = 'decEntry';
      return;
    }
    switch (
      error.code // 'Failure' messages
    ) {
      case WRONG_PIN:
  ...
  _validateTransport(payload) {
    if (typeof payload.type !== 'undefined' && typeof payload.version !== 'undefined') {
      if (payload.type !== 'bridge' && payload.type !== 'WebUsbPlugin') {
        ...
        payload.type === 'bridge' &&
        !this._versionCompare(payload.version, MINIMAL_VERSION)
      ) {
    ...
    this.trezorConnect
      ...
      .then(result => {
        ...
        if (result.success) {
          ...
          this.bgStore.encryptionKey = new Buffer(
